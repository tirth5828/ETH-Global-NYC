...
with tab1:
    ...
    if not st.session_state.wallet_connected:
        ...
        if balance_data:
            ...
            with col2:
                st.subheader("🏦 DeFi Positions (Live Data)")
                
                if defi_data:
                    # Create bar chart for DeFi APYs
                    protocols = [pos['protocol'] for pos in defi_data]
                    apys = [pos['apy'] for pos in defi_data]
                    
                    fig_bar = px.bar(x=protocols, y=apys, title="DeFi APY by Protocol", 
                                   labels={'x': 'Protocol', 'y': 'APY (%)'})
                    fig_bar.update_traces(marker_color='lightblue')
                    st.plotly_chart(fig_bar, use_container_width=True)
                    
                    # Real DeFi positions table
                    defi_df = pd.DataFrame([
                        {
                            'Protocol': pos['protocol'],
                            'Pool': pos['pool'],
                            'Value USD': f"${pos['value_usd']:,.2f}",
                            'APY': f"{pos['apy']:.2f}%",
                            'Status': pos['status']
                        }
                        for pos in defi_data
                    ...
                    st.info("No DeFi positions found for this wallet.")
            
            # Real-time price ticker
            st.subheader("📈 Live Token Prices")
            prices = get_token_prices(['ETH', 'BTC', 'USDC', 'LINK'])
            ...
            for i, (symbol, price) in enumerate(prices.items()):
                with price_cols[i]:
                    ...
                    st.metric(symbol, f"${price:,.2f}", f"{change:+.2f}%", 
...
# with tab4:
#     st.header("🔗 Blockchain Transaction History")
    
#     if not st.session_state.wallet_connected:
#         st.warning("Please connect your wallet to view transaction history.")
#     else:
#         st.info("🚧 Real-time transaction history integration coming soon! This will fetch actual blockchain transactions for your wallet.")
        
#         # For now, show demo transactions but indicate they're simulated
#         st.markdown("### 📝 Sample Transaction Data (Demo)")
        
#         # Transaction filters
#         col1, col2, col3 = st.columns(3)
#         with col1:
#             transaction_type = st.selectbox("Transaction Type", ["All", "Swap", "Supply", "Borrow", "LP", "Withdraw"])
#         with col2:
#             time_filter = st.selectbox("Time Period", ["All Time", "Last 24h", "Last Week", "Last Month"])
#         with col3:
#             status_filter = st.selectbox("Status", ["All", "Success", "Pending", "Failed"])
        
#         # Generate sample transactions (in production, this would fetch real data)
#         sample_transactions = [
#             {
...
#                 'value_usd': 7125.00,
#                 'status': 'Success',
#                 'hash': f'0x{np.random.randint(0, 16**64):064x}'[:42] + '...'
#             },
#             {
...
#                 'Date': tx['date'].strftime('%Y-%m-%d %H:%M'),
#                 'Type': tx['type'],
#                 'Details': tx.get('from', '') + ' → ' + tx.get('to', '') if 'from' in tx else tx.get('protocol', '') + ' - ' + tx.get('asset', ''),
#                 'Amount': tx['amount'],
#                 'Status': tx['status'],
#                 'Hash': tx['hash']
#             })
...
#                     'Transaction Hash',
#                     help='Click to view on blockchain explorer',
...
#             daily_counts = tx_df.groupby(tx_df['Date'].dt.date).size().reset_index(name='count')
            
#             fig_freq = px.bar(daily_counts, x='Date', y='count', 
